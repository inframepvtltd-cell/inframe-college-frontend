...
const API_BASE =
  process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:7000/api/v1";
...
export const extractContactInfo = (contacts: ContactInfoItem[]) => {
  return {
    ...
    collegeName:
      getContactValue(contacts, "name") ||
      getContactValue(contacts, "college_name"),

    // Phones
    admissionPhone:
      getContactValue(contacts, "admission_mob") ||
      getContactValue(contacts, "admission_phone"),
    adminPhone:
      getContactValue(contacts, "admin_mob") ||
