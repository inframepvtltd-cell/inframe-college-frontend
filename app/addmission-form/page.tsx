...
import ThankYouStep from "./components/ThankYou";
import { useRouter } from "next/navigation";
import ProgramAndPaymentForm from "./components/Form/ProgramSelectionForm";
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      const processedData = {
        ...formData,
        ...parsedData,
        educationDetails: parsedData.educationDetails || [...DEFAULT_EDUCATION_ROWS],
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1: // Personal Details
        ...
        if (!formData.phone?.trim() || !/^\d{10}$/.test(formData.phone)) errors.push("Valid phone number (10 digits) is required");
        if (!formData.gender) errors.push("Gender is required");
        if (!formData.dateOfBirth) errors.push("Date of birth is required");
        if (!formData.permanentAddress?.trim()) errors.push("Permanent address is required");
        if (!formData.aadharNumber?.trim() || !/^\d{12}$/.test(formData.aadharNumber)) errors.push("Valid Aadhar number (12 digits) is required");
      ...
      case 2: // Education Details
        const importantEducation = formData.educationDetails.filter(edu =>
          ['10th', '12th', '10', '12'].includes(edu.classLevel.toLowerCase().replace('th', '').replace('nd', ''))
        );

        // Check 10th and 12th are present
        const has10th = importantEducation.some(edu =>
          ['10th', '10'].includes(edu.classLevel.toLowerCase().replace('th', ''))
        );
        const has12th = importantEducation.some(edu =>
          ['12th', '12'].includes(edu.classLevel.toLowerCase().replace('th', '').replace('nd', ''))
        );
        ...
        formData.educationDetails.forEach((edu, index) => {
          if (['10th', '12th', '10', '12'].includes(edu.classLevel.toLowerCase().replace('th', '').replace('nd', ''))) {
            if (!edu.institutionName?.trim()) errors.push(`Education ${index + 1}: Institution name is required for ${edu.classLevel}`);
            if (!edu.stream?.trim()) errors.push(`Education ${index + 1}: Stream is required for ${edu.classLevel}`);
            if (!edu.yearOfPassing?.trim()) errors.push(`Education ${index + 1}: Year of passing is required for ${edu.classLevel}`);
            if (!edu.grade?.trim()) errors.push(`Education ${index + 1}: Grade is required for ${edu.classLevel}`);
            if (!edu.document) errors.push(`Education ${index + 1}: Document upload is required for ${edu.classLevel}`);
          }
        });
        break;
      ...
      case 3: // Documents
        ...
        const hasAadhar = formData.documents.some(doc =>
          doc.fileType === 'aadhar' && doc.file
        );
        const hasSignature = formData.documents.some(doc =>
          doc.fileType === 'signature' && doc.file
        );
        ...
        formData.documents.forEach((doc, index) => {
          if (doc.fileType && !doc.file) {
            errors.push(`Document ${index + 1}: ${doc.fileType} file is required`);
          }
        });
        break;

      case 4: // Terms & Conditions
        if (!formData.termsAccepted) errors.push("You must accept terms and conditions");
        break;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const applicationId = response.data?.studentId || `APP-${Date.now().toString().slice(-6)}`;
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-8">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <ul className="text-xs text-red-700 mt-1 space-y-1">
                        {stepErrors[currentStep].slice(0, 3).map((error, idx) => (
                          <li key={idx}>• {error}</li>
                        ))}
                        {stepErrors[currentStep].length > 3 && (
                          <li>• ...and {stepErrors[currentStep].length - 3} more</li>
                        )}
                      </ul>
                    </div>
                  </div>
              ...
              {/* <div className="mt-6 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-800">
                  <span className="font-medium">Note:</span> For offline payment, submit form and visit campus.
                </p>
              </div> */}
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-red-600 mt-1">
                        {stepErrors[currentStep].length} issue{stepErrors[currentStep].length > 1 ? 's' : ''} need{stepErrors[currentStep].length > 1 ? '' : 's'} attention
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-4">
                      {currentStep < steps.length ? (
                        <button
                          ...
                          className={`px-6 py-3 font-semibold rounded-xl shadow-lg transform transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center gap-2 ${stepErrors[currentStep]?.length > 0
                        ...
                        <button
                          ...
                          disabled={isSubmitting || stepErrors[currentStep]?.length > 0}
                          className={`px-8 py-3 font-semibold rounded-xl shadow-lg transform transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center gap-2 ${stepErrors[currentStep]?.length > 0
                            ? "bg-gray-400 text-white"
